...
`npm run start` は `src/` から `dist/index.html` を生成し、3631 番ポートで配信します。
`src/index.md` や `src/style.css` を保存するとブラウザが自動でリロードします。
...
| パス | 内容 |
| --- | --- |
| `src/index.md` | 本文（唯一の原稿。Markdown + 必要に応じて生 HTML） |
| `src/style.css` | 見た目。自由に編集してよい（初期状態は Google ドキュメントの書き出しに合わせてある） |
| `src/images/` | 画像 |
| `dist/` | 生成物（git 管理外） |
| `google_docs/` | 移行元の Google ドキュメント書き出し（保存用） |
| `tools/gdocs_to_md.py` | `google_docs/index.html` → `src/index.md` の変換スクリプト（移行用。通常は使わない） |
